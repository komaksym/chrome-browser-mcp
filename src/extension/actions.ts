...
export function performFileUpload(action: FileUploadAction) {
  const normalize = (value: string) => value.replace(/\s+/g, " ").trim().toLocaleLowerCase();
  ...
  const resolveTarget = (target: string): HTMLInputElement => {
    let selectorMatches: HTMLInputElement[] = [];
    try {
      selectorMatches = Array.from(document.querySelectorAll(target)).filter(
        (element): element is HTMLInputElement =>
          element instanceof HTMLInputElement && element.type.toLocaleLowerCase() === "file",
      );
    } catch {
      // Not a CSS selector; fall back to human-readable file-input names.
    }
    const selectorMatch = selectorMatches[0];
    if (selectorMatches.length === 1 && selectorMatch) return selectorMatch;
    if (selectorMatches.length > 1) throw new Error(`AMBIGUOUS_TARGET: ${target}`);
