...
Current patch version: **0.1.22**. Every patch must bump this version; CI rejects patches that do not.
...
The visible extension version must change on every patch. For this patch it must show **0.1.22**. If it still shows an older version, the pulled runtime was not applied.
...
For the opt-in isolated headed-Chrome preflight used by the live ChatGPT smoke suite, see [`docs/LIVE_CHATGPT_SMOKE.md`](docs/LIVE_CHATGPT_SMOKE.md). It uses a dedicated persistent Chrome profile, port, and tunnel profile and does not submit a ChatGPT prompt.
