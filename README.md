...
Current patch version: **0.1.5**. Every patch must bump this version; CI rejects patches that do not.
...
The visible extension version must change on every patch. For this patch it must show **0.1.5**. If it still shows an older version, the pulled runtime was not applied.
